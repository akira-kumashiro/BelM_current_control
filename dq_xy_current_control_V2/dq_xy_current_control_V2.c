...
#define Ki_gain2p 1.0//2pIゲイン調整用
#define Kp_gain4p 1.0//4pPゲイン調整用
#define Ki_gain4p 1.0//4pIゲイン調整用
#define MAX_RPM 10.0e+3//回転数計算用のレンジ設定
//#define ki_dc 1.70//支持力係数[N/A]の直流成分
//#define ki_ac_amp 0.093//支持力係数[N/A]の回転角度変動成分
//#define ki_dc 0.588//(支持力係数[N/A])^(-1)の直流成分
//#define ki_dc 0.9//(支持力係数[N/A])^(-1)の直流成分
#define ki_ac_amp 0.0547
...
volatile FLOAT32 enc_theta_offset=(-51.246-7.488);//エンコーダ角度オフセット値　0～2pi[rad]
volatile FLOAT32 enc_hall_diff = 0;
/*--------------------enc関連設定ここまで--------------------*/
...
//volatile FLOAT32 fxA_ref = 0.0, fyA_ref = 0.0;//ラジアル力の指令値[N]
...
volatile INT32 enable_PI_controlA = 1, enable_PI_controlB = 0, enable_dq_current_direct_control=1;//I制御停止用のVdcチェック変数 VdcがVdc_thresよりも大きいと1、それ以外0
...
volatile INT32 enable_decoupling_mode = 0;//電圧干渉計算ON/OFF
...
FLOAT32 KPts = 12.8359, KIts = 141.0516, KDts = 0.29202;//角度位置制御のPIDゲイン(角度)
FLOAT32 KPtr = 0.090878, KItr = 0.028874, KDtr = 0.071506;//角度位置制御のPIDゲイン(速度)
...
FLOAT32 KPsus = 139145, KIsus = 10770510, KDsus = 449, Tfsus = 1 / (PI(2)*5.0e+2);//磁気支持制御のPIDゲイン 20221223 LSRあり
...
FLOAT32 triggerWave=-1.0;
FLOAT32 triggerPeriod_us=1.0e+6;//usec
/*--------------------電流制御用変数ここまで--------------------*/
...
void getAdStatusSensor(void)
{
	...
	enc_theta_calc=radianLimit(enc_theta_raw+enc_theta_offset*PI(1)/180);//エンコーダ角度のオフセットを計算
	enc_hall_diff = enc_theta_calc - hall_theta_calc;
	pro_da_out(DA_CH_0, hall_theta_calc);
	...
	triggerWave+=(PERIOD/SAMPL)/triggerPeriod_us;
	if(triggerWave>1.0)
		triggerWave=-1.0;
}
...
void current_controlA(void) 
{
	...
	uvw2dq(iuA, iwA, hall_theta_calc+theta_sus_0*PI(1)/180, &ixA, &iyA);
	...
	if(enable_PI_controlA)
	{
		//pi_control(ixA, ixA_ref, KPx4p, KIx4p, ixA_max, &ixA_Pout, &ixA_Iout, &ixA_Itmp);
		//pi_control(iyA, iyA_ref, KPy4p, KIy4p, iyA_max, &iyA_Pout, &iyA_Iout, &iyA_Itmp);
		vxA_ref = pi_control_release(ixA, ixA_ref, KPx4p, KIx4p, ixA_max, &ixA_Itmp);
		vyA_ref = pi_control_release(iyA, iyA_ref, KPy4p, KIy4p, iyA_max, &iyA_Itmp);
...
void current_controlB(void) 
{
	...
	{
		setBThetaGeinZero();
		if(enable_dq_current_direct_control==0)
		{
			idB_ref=0;
			iqB_ref=0;			
		}
    ...
    // dq軸干渉項の計算
    _vodB = -reference_rpm_calc / 60 * PI(2) * Lq_2p * _iqB;
    _voqB = reference_rpm_calc / 60 * PI(2) * (PSY_a + Ld_2p * _idB);
	...
	vdB_ref += _vodB * enable_decoupling_mode;
	vqB_ref += _voqB * enable_decoupling_mode;
...
void positionControlB(void)
{
	const FLOAT32 filter_factor = 1 / (PI(2) * 100 / Fcarrier + 1);//参考 https://haizairenmei.com/2018/10/27/arduino_noise/
...
void positionControlA(void)
{
	...
	FLOAT32 filter_factor = Tfsus * Fcarrier / (1 + Fcarrier * Tfsus);//LPF時定数から計算
	//const FLOAT32 xy_filter_factor = 1 / (Fcarrier * PI(2) * rotational_speed / 60 / 10 + 1);
	FLOAT32 xsus_Dcalc, ysus_Dcalc;
	FLOAT32 fxA_ref = 0.0, fyA_ref = 0.0;//ラジアル力の指令値[N]
	...
	/*fxA_ref = xsus_Pout + xsus_Iout + xsus_Dout;
	fyA_ref = ysus_Pout + ysus_Iout + ysus_Dout;
	ixA_ref = fxA_ref / (ki_dc);
	iyA_ref = fyA_ref / (ki_dc);
	//ixA_ref = mylimit(fxA_ref / (ki_dc + ki_ac_amp * mwcos(2.0 * hall_theta_calc * PI(1) / 180.0)), ixA_max);	
	//iyA_ref = mylimit(fyA_ref / (ki_dc - ki_ac_amp * mwcos(2.0 * hall_theta_calc * PI(1) / 180.0)), iyA_max);
	ixA_ref = mylimit(ixA_ref, ixA_max);	
	iyA_ref = mylimit(iyA_ref, iyA_max);*/
	
	//ixA_ref = mylimit(((xsus_Pout + xsus_Iout + xsus_Dout)*(1+ki_ac_amp*mwcos(2.0*hall_theta_calc*PI(1)/180))), ixA_max);
	//iyA_ref = mylimit(((ysus_Pout + ysus_Iout + ysus_Dout)*(1-ki_ac_amp*mwcos(2.0*hall_theta_calc*PI(1)/180))), iyA_max);
...
int main( void )
{
	...
	pro_da_set_range(DA_CH_0,PI(1));
	pro_da_set_range(DA_CH_1,PI(1));
