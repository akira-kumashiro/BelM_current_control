...
volatile FLOAT32 idA_ref = 0.0, iqA_ref = 0.0, idB_ref = 1.0, iqB_ref = 0.0;// dq軸電流の指令値[A]
...
volatile INT32 enable_I_controlA = 0, enable_I_controlB = 0;//I制御停止用のVdcチェック変数 VdcがVdc_thresよりも大きいと1、それ以外0
/*--------------------電流制御用変数ここまで--------------------*/
...
void getHallValue(void)
{
	...
	pro_da_out(DA_CH_0,hallT);							/* Output data for viewing waveforms in DAC					*/
	pro_da_out(DA_CH_2,hallOUT);
}
...
{
	/* AD変換結果の取得 */
	ad_in( CASCADE_SEQ, AD_GROUP_6, &hallA, &hallB );				/*	Reading A/D conversion output data of channel group6										*/
	ad_in( CASCADE_SEQ, AD_GROUP_7, &hallC, &hallD );				/*	Reading A/D conversion output data of channel group7										*/
}

void getAdStatusAinv(void)
{
	ad_in( CASCADE_SEQ, AD_GROUP_1, &iuA_raw, &iwA_raw );				/*	Reading A/D conversion output data of channel group1										*/
	iuA=i_raw_converterA(iuA_raw);
	iwA=i_raw_converterA(iwA_raw);

	ad_in( CASCADE_SEQ, AD_GROUP_2, &vdcA_raw, &idcA_raw );				/*	Reading A/D conversion output data of channel group2										*/
...
void getAdStatusBinv(void)
{
	ad_in( CASCADE_SEQ, AD_GROUP_0, &iuB_raw, &iwB_raw );				//	Reading A/D conversion output data of channel group0										
	iuB=i_raw_converterB(iuB_raw);
	iwB=i_raw_converterB(iwB_raw);

	ad_in( CASCADE_SEQ, AD_GROUP_3, &vdcB_raw, &idcB_raw );				/*	Reading A/D conversion output data of channel group2										*/
...
void current_controlB(void) 
{
    ...
    ab2dq(iaB, ibB, hallOUT, &idB, &iqB);

    // PI制御 d軸
    idB_ref = mylimit(idB_ref, idB_max);
    // d軸電流 P制御
    idB_err = idB_ref - idB;
    idB_Pout = KPd2p * idB_err * (FLOAT32)enable_I_controlB;
    // d軸電流 I制御
    idB_Itmp += idB_err * (FLOAT32)enable_I_controlB;
    ...
    // q軸電流 P制御
    iqB_err = iqB_ref - iqB;
    iqB_Pout = KPq2p * iqB_err * (FLOAT32)enable_I_controlB;
    // q軸電流 I制御
    iqB_Itmp += iqB_err * (FLOAT32)enable_I_controlB;
    ...
    // uvw電圧を出力
    // vd, vq → vu, vv, vw (絶対変換)
    dq2ab(vdB_ref, vqB_ref, hallOUT, &vaB, &vbB);
...
int main( void )
{
	...
	ad_init( CONTINUOUS_SCAN );							/*	Initializing A/D conversion module and startign the conversion on Continuous scan mode		*/
	...
	ad_set_range(AD_GROUP_4, AD_RANGE, AD_RANGE);
